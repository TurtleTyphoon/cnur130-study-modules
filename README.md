...
- **Objective coach** (each module → Objective path, and a **Learn it →** button beside every learning outcome in Modules 1–4): each objective has Teach (explanation, key points, diagram, “On the unit”, exam trap, memory hook, page references), Check (recall questions), Apply (clinical scenarios: multiple choice, select all that apply, matrix) and an optional Explain it back. A wrong answer shows why that choice is wrong, with **Try again** and **Show answer** buttons, so you can retry it straight away instead of waiting for the result; a retried question doesn’t count toward mastery that round. Getting every Check and Apply question right on the first try masters the objective; reviews come due after 1, 3 and 7 days, and a missed review drops it back to Learning. Mastered objectives count toward Exam readiness for modules that have coach content (40% accuracy, 30% questions tried, 30% objectives mastered; review due counts half). Content lives in `coachData()` keyed by module and outcome id; each module’s `_src` maps its chapters to the reading cited on the Teach step, so other modules can be added as data only.
